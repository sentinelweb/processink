...
void main(void) {
    ...
    float n = surface3(vec3(position*sin(u_time*0.1), u_time * 0.05)*mat3(1, 0, 0, 0, .8, .6, 0, -.6, .8), 0.9);
    float n2 = surface3(vec3(position*cos(u_time*0.1), u_time * 0.04)*mat3(1, 0, 0, 0, .8, .6, 0, -.6, .8), 0.8);
    float lum = length(n);
    float lum2 = length(n2);

    vec3 tc = pow(vec3(1.0-lum), vec3(sin(position.x)+cos(u_time)+4.0, 8.0+sin(u_time)+4.0, 8.0));
    vec3 tc2 = pow(vec3(1.1-lum2), vec3(5.0, position.y+cos(u_time)+7.0, sin(position.x)+sin(u_time)+2.0));
