...
{
    return texture(texture, (vec2(x)+0.5)/256.0, -100.0);
}
...
vec4 Rand(in int x)
{
    ...
    return texture(texture, uv, -100.0);
}

void main()
{
    vec3 ray;
    ray.xy = 2.0*(gl_FragCoord.xy-u_resolution.xy*.5)/u_resolution.x;
    ray.z = 1.0;

    float offset = u_time*.5;
    float speed2 = u_mouse.x/u_resolution.x*4;
